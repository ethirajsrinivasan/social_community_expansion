...
def sample_graph(G_linklist,node_number,degree_sequence,starting_node,sample_rate=None,biased=True):
    ...
    for i in range(new_graph_size):
        for j in range(new_graph_size):
                if i in map_dict.keys() and j in map_dict.keys():
                    a = map_dict[i]
                    b = map_dict[j]
...
#########################################################################################
    ...
    parser.add_option("-f", "--input network file", dest="network_file", default="../example/fb/graph",
                      help="input file of edge list for clustering [default: example_graphs/fb/graph]")

    parser.add_option("-g", "--input community ground truth file", dest="groundtruth_community_file", default="../example/fb/community",
                      help="input file of ground truth community membership [default: example_graphs/fb/community]")
    ...
    parser.add_option("--sd", "--input seed set file", dest="seed_set_file", default="../example/fb/seed",
                      help="input file of initial seed set [default: example_graphs/fb/seed]")
    ...
    with open(seed_set_file) as fin:
        for line in fin.readlines():
            if not line.strip().startswith("#"):
                ...
                break
    seedset = np.array(seedset)
    
    print(seedset)
    # modify the "test_comm" if you want to test some other ground truth communities
    # test_comm = np.array([ 6, 7 ,8, 47, 131, 132, 140, 148, 166, 171, 191, 194, 209, 212, 214, 271, 284, 349, 361, 375, 396, 417, 429])
    # test_comm = np.array([6, 7, 8, 47, 131, 132, 140, 148, 166, 171, 191, 194, 209, 212, 214, 271, 284, 349, 361, 375, 396, 417, 429])
    # test_comm = np.array([1, 9, 22, 45, 49, 62, 71, 85, 86, 93, 102, 112, 121, 123, 136, 138, 139, 141, 144, 150, 151, 152, 154, 164, 167, 174, 185, 188, 190, 193, 195, 204, 210, 213, 219, 221, 226, 235, 237, 239, 241, 242, 256, 266, 269, 281, 288, 294, 303, 309, 317, 356, 384, 385, 388, 402, 405, 409, 412, 418, 428, 435, 436, 444, 447, 477, 480, 483, 484, 494, 495, 499, 504, 511, 517, 518, 519, 521, 524, 536, 540, 555, 557, 567, 572, 579, 580, 582, 590, 600, 604, 613, 623, 627, 632, 638, 639, 640, 641, 642, 648, 649, 651, 656, 663, 665])
    test_comm = np.array([0, 4, 16, 17, 18, 21, 25, 26, 35, 40, 43, 46, 50, 51, 52, 54, 55, 57, 67, 74, 75, 77, 83, 87, 91, 92, 95, 96, 99, 101, 104, 105, 110, 113, 114, 117, 120, 122, 134, 168, 169, 175, 180, 181, 189, 222, 240, 250, 270, 275, 277, 307, 320, 321, 322, 323, 324, 325, 326, 327, 347, 348, 351, 352, 353, 355, 358, 368, 371, 416, 666])
    test_comm = test_comm
